class AddCompactedToMusicServiceAlbums < ActiveRecord::Migration[6.0]
  def change
    ...
    add_index :apple_music_albums, [:apple_music_id, :status], unique: true
    add_index :spotify_albums, [:spotify_id, :status], unique: true
    remove_index :apple_music_tracks, [:apple_music_id]
    remove_index :spotify_tracks, [:spotify_id]
    add_index :apple_music_tracks, [:apple_music_id, :status], unique: true
    add_index :spotify_tracks, [:spotify_id, :status], unique: true
  end
end
